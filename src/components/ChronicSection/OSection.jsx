...
const OBESITY_BADGE_COLORS = {
  '저체중':    'bg-blue-100 text-blue-700',
  '정상':      'bg-green-100 text-green-700',
  '과체중':    'bg-yellow-100 text-yellow-700',
  '비만 1단계': 'bg-orange-100 text-orange-700',
  '비만 2단계': 'bg-red-100 text-red-700',
  '비만 3단계': 'bg-red-200 text-red-800',
};
...
function BodySection({ data, set, bmi, selectedDiseases }) {
  const showWaist = selectedDiseases.includes('Obesity') || selectedDiseases.includes('MASLD');
  const showObesityBadge = selectedDiseases.includes('Obesity');
  const obesityClass = bmi && showObesityBadge ? getObesityClass(bmi) : null;
  ...
  return (
    <div className="space-y-0.5">
      ...
      <div className="flex items-center gap-2 flex-wrap py-1">
        ...
        {obesityClass && (
          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${OBESITY_BADGE_COLORS[obesityClass]}`}>
            {obesityClass}
          </span>
        )}
      </div>
      {showWaist && (
        <div className="flex items-center gap-2 flex-wrap py-1">
          <span className="text-sm text-slate-600 shrink-0">허리둘레</span>
          <NumInput value={data.waist} onChange={v => set('waist', v)} placeholder="85" step="0.1" className="w-18" label="허리둘레 (cm)" />
          <span className="text-xs text-slate-400">cm</span>
        </div>
      )}
    </div>
...
function OsteoporosisOSection({ data, set }) {
  return (
    <div className="space-y-0.5">
      ...
      <div className="flex items-center gap-2 flex-wrap py-1">
        <span className="text-sm text-slate-600 shrink-0">Vit D</span>
        <NumInput value={data.vitD} onChange={v => set('vitD', v)} placeholder="30" step="0.1" className="w-18" label="비타민 D (ng/mL)" />
...
function MASLDOSection({ data, set, showVitD }) {
  return (
    <div className="space-y-0.5">
      <SectionLabel label="MASLD" />
      <div className="flex items-center gap-2 flex-wrap py-1">
        <span className="text-sm text-slate-600 shrink-0">AST</span>
        <NumInput value={data.ast} onChange={v => set('ast', v)} placeholder="30" label="AST (U/L)" />
        <span className="text-xs text-slate-400">U/L</span>
        <span className="text-sm text-slate-600 shrink-0 ml-2">ALT</span>
        <NumInput value={data.alt} onChange={v => set('alt', v)} placeholder="35" label="ALT (U/L)" />
        <span className="text-xs text-slate-400">U/L</span>
      </div>
      {showVitD && (
        <div className="flex items-center gap-2 flex-wrap py-1">
...
function generateOText(data, selectedDiseases, bmi, egfr, ascvdResult, activeFactorNames, kdigoResult, htnRisk) {
  ...
  if (data.height || data.weight) {
    ...
    if (bmi) {
      body += `BMI ${bmi} kg/m\u00B2`;
      if (selectedDiseases.includes('Obesity')) {
        const cls = getObesityClass(bmi);
        if (cls) body += ` (${cls})`;
      }
    }
    if (data.waist && (selectedDiseases.includes('Obesity') || selectedDiseases.includes('MASLD'))) {
      body += ` 허리둘레 ${data.waist}cm`;
    }
  ...
  // Osteoporosis
  if (selectedDiseases.includes('Osteoporosis')) {
    const parts = [];
    if (data.osteo_tscore) parts.push(`BMD T-score ${data.osteo_tscore}`);
    if (data.vitD) parts.push(`Vit D ${data.vitD} ng/mL`);
  ...
  // MASLD
  if (selectedDiseases.includes('MASLD')) {
    const parts = [];
    if (data.ast) parts.push(`AST ${data.ast}`);
    if (data.alt) parts.push(`ALT ${data.alt}`);
    if (parts.length > 0) {
      let maslLine = parts.join(' / ') + ' U/L';
      if (data.vitD && !selectedDiseases.includes('Osteoporosis')) {
        maslLine += ` / Vit D ${data.vitD} ng/mL`;
      }
      lines.push(maslLine);
    } else if (data.vitD && !selectedDiseases.includes('Osteoporosis')) {
      lines.push(`Vit D ${data.vitD} ng/mL`);
    }
  }

  // Hb (항상 표시, 입력 시만 출력)
  if (data.hb) {
    lines.push(`Hb ${data.hb} g/dL`);
  }
...
export default function OSection({
  ...
  // prefill: prefillTrigger 변경 시만 실행 (버튼 클릭 시)
  useEffect(() => {
    if (!prefillData || !prefillTrigger) return;
    setFormData(prev => ({
      ...prev,
      vs_sbp: prefillData.sbp || prev.vs_sbp,
      vs_dbp: prefillData.dbp || prev.vs_dbp,
      vs_pr:  prefillData.pr  || prev.vs_pr,
      vs_bt:  prefillData.bt  || prev.vs_bt,
      height: prefillData.height || prev.height,
      weight: prefillData.weight || prev.weight,
      waist:  prefillData.waist  || prev.waist,
      dm_hba1c: prefillData.hba1c || prev.dm_hba1c,
      dm_bst:   prefillData.bst   || prev.dm_bst,
      lipid_tc:  prefillData.tc  || prev.lipid_tc,
      lipid_tg:  prefillData.tg  || prev.lipid_tg,
      lipid_hdl: prefillData.hdl || prev.lipid_hdl,
      lipid_ldl: prefillData.ldl || prev.lipid_ldl,
      ckd_bun: prefillData.bun || prev.ckd_bun,
      ckd_cr:  prefillData.cr  || prev.ckd_cr,
      ckd_acr: prefillData.acr || prev.ckd_acr,
      osteo_tscore: prefillData.tscore || prev.osteo_tscore,
      thyroid_tsh: prefillData.tsh || prev.thyroid_tsh,
      thyroid_ft4: prefillData.ft4 || prev.thyroid_ft4,
      vitD: prefillData.vitD || prev.vitD,
      ast:  prefillData.ast  || prev.ast,
      alt:  prefillData.alt  || prev.alt,
      hb:   prefillData.hb   || prev.hb,
    }));
  ...
  }, [formData, selectedDiseases, bmi, egfr, ascvdResult, activeFactorNames, kdigoResult, htnRisk, ckdStageLabel, onChange, onFormDataChange]);

  const hasThyroid = selectedDiseases.includes('Hypothyroidism') || selectedDiseases.includes('Hyperthyroidism');
  const hasMASLD = selectedDiseases.includes('MASLD');
  const hasOsteoporosis = selectedDiseases.includes('Osteoporosis');
  ...
  return (
    <div className="space-y-4">
      ...
      <BodySection data={formData} set={set} bmi={bmi} selectedDiseases={selectedDiseases} />
      ...
      {hasMASLD && (
        <MASLDOSection data={formData} set={set} showVitD={!hasOsteoporosis} />
      )}
      ...
      {/* Hb: 항상 표시 */}
      <div className="space-y-0.5">
        <SectionLabel label="혈액" />
        <div className="flex items-center gap-2 flex-wrap py-1">
          <span className="text-sm text-slate-600 shrink-0">Hb</span>
          <NumInput value={formData.hb} onChange={v => set('hb', v)} placeholder="13.5" step="0.1" className="w-18" label="헤모글로빈 (g/dL)" />
